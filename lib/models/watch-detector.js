...
function watchmanVersionSatisfiesSemver(currentVersion, expectedSemanticVersion) {
  let currentSatisfiesExpected = false;
  try {
    currentSatisfiesExpected = semver.satisfies(currentVersion, expectedSemanticVersion);
  } catch (e) {
    const dateStr = currentVersion
      .replace(/\./g, '/')
      .split('/')
      .slice(0, -1)
      .join('/');
    const isValidDate = !isNaN(new Date(dateStr));
    // preserve semver.satisfies behavior by rethrowing
    if (!isValidDate) { throw e; }
    // if current version can be parsed as a date,
    // it is after the switch away from semver
    currentSatisfiesExpected = true;
  }
  return currentSatisfiesExpected;
}
...
class WatchPreference {
  ...
  watchmanWorks(details) {
    ...
    this._watchmanInfo.canNestRoots = watchmanVersionSatisfiesSemver(
      details.version,
      '>= 3.7.0'
    );
  }
  ...
  */
  checkWatchman() {
    ...
    try {
      ...
      if (watchmanVersionSatisfiesSemver(version, '>= 3.0.0')) {
        ...
        result.watchmanWorks({
          version,
          canNestRoots: watchmanVersionSatisfiesSemver(version, '>= 3.7.0'),
        });
      } else {
